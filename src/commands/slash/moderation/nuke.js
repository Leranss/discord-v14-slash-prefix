...
module.exports = {
    ...
    async execute(interaction) {
        if (!interaction.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
            return interaction.reply({ content: "Kanalları Yönet iznin yok." });
        }
        
        if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageChannels)) {
            return interaction.reply({ content: "Kanalları Yönet iznim yok." });
        }
            ...
            channel.send({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`Bu kanal ${interaction.user.toString()} tarafından yeniden oluşturuldu.`)
